...
class throughtopic extends React.Component {
  constructor(props) {
      super(props);  
      this.state = {
        columns:[{
          ...
          render:(text,record)=>(
            <span>{record.serviceType==1002?'动态':''}{record.serviceType==1003?'活动':''}{record.serviceType==1004?'新闻资讯':''}{record.serviceType==1006?'话题跟贴':''}</span>
          )
          },{
            title: '审核人',
            ...
            <Row>
                ...
                <Col span={8}>
                     <FormItem label={`类型`}
                            ...
                            onChange={v=>{this.setState({formValue:Object.assign(this.state.formValue,{searchType:v})})}}
                            filterOption={(input, option) => option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
                            style={{width:190}}
                            >
                            <Option value={0}>全部</Option>
                            <Option value={1}>话题跟帖</Option>
...
const OpenWithForm = Form.create()(throughtopic);

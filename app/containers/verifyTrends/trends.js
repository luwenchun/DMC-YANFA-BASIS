...
class trend extends React.Component {
  ...
  componentDidMount() {
    ...
    Http.get('getCourseInfo', webDetail, result => {
      this.setState({
        getCourseInfo: result

      }, () => {
        if (result['courseImages']) {
          if (result['courseImages'] && result['courseImages'].split(",")) {
            _that.setState({
              imgs: result['courseImages'].split(",")
            })
          }
        }
      })
  ...
  render() {
    ...
    const data = [];
    console.log(this.state.getCourseInfo)
    for (let i = 0; i < 46; i++) {
      data.push({
        key: i,
        name: `Edward King ${i}`,
    ...
    return (
      <div className={'wrap'} style={{ 'padding': '12px' }}>
        ...
        <Row gutter={24} className={'all'}>
          <Col className={'alls'}>
            <ul>
              <li><span>审核类型</span>：{this.state.getCourseInfo['type']}</li>
              <li><span>发布人</span>：{this.state.getCourseInfo['userName']}</li>
              <li><span>手机号</span>：{this.state.getCourseInfo['phone']}</li>
              <li><span style={{ float: 'left', marginRight: '10px' }}>动态图片：</span>
                <p className={'img-wrapper'} style={{paddingLeft:'80px'}}>
                  {
                    this.state.imgs.map((item, index) => {
                      return (
                        <span key={index} style={{ float: 'left', margin: "0 10px 10px 0", width: '100px'}}><img src={item} /></span>
                      )
                    })
                  }
                </p>
          ...
          <Col>
            <div>
              正文：
              <div className={'imgWidth'} dangerouslySetInnerHTML={{__html:this.state.getCourseInfo['courseText']}}></div>
                         {/* <TextArea rows={4} value={this.state.getCourseInfo['courseText']} disabled style={{ color: 'black' }} /> */}
            </div>
          </Col>
        </Row>
  ...
  two(a) {
    ...
    const code = {
      ...
      reviewStatus: a,
      dealerCode: this.state.getCourseInfo['dealerCode'],
      phone: this.state.getCourseInfo['phone']
    }
...
const OpenWithForm = Form.create()(trend);
